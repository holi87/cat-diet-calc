    ...
    <div>
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        <button
          ...
          className={`text-xs px-3 py-1 rounded-full font-medium border ${!filterCategory ? 'bg-brand-500 text-white border-brand-500' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700'}`}
        >
          Wszystkie
        </button>
        {categories.map((c) => (
          <button
            ...
            className={`text-xs px-3 py-1 rounded-full font-medium border ${filterCategory === c.value ? 'bg-brand-500 text-white border-brand-500' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700'}`}
          >
            {c.label}
          </button>
        ))}
        <button
          ...
          className={`text-xs px-3 py-1 rounded-full font-medium border ml-auto ${showArchived ? 'bg-gray-500 text-white border-gray-500' : 'bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-500 border-gray-200 dark:border-gray-700'}`}
        >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10">
          ...
          <div className="text-base text-gray-500 dark:text-gray-400 font-medium mb-1">Brak produktów</div>
          <div className="text-sm text-gray-400 dark:text-gray-500">Kliknij „+ Dodaj" aby dodać pierwszy produkt</div>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map((food) => (
            <div
              key={food.id}
              className={`bg-white dark:bg-gray-800 rounded-xl border shadow-sm dark:shadow-gray-900/30 px-4 py-3 flex items-center gap-3 ${food.archived ? 'opacity-50 border-gray-100 dark:border-gray-700' : 'border-gray-100 dark:border-gray-700'}`}
            >
              <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${CATEGORY_BADGE_COLORS[food.category as FoodCategory]}`}>
                {CATEGORY_LABELS[food.category as FoodCategory]}
              </span>
